---
title: "Ammonia (N) Rate of Change Analysis"
format: html
params:
...
samples_df <- df %>%
  # construct datetime column from Month,Day,Year
  mutate(datetime = ymd(paste(Year, Month, Day))) %>%
  # convert Value to numeric
  mutate(Value = as.numeric(Value)) %>%
  # drop any with empty required columns
  filter(!is.na(Site) & !is.na(datetime) & !is.na(Value)) %>%
...
sample_stats_df <- samples_df %>%
  ...
  summarize(
    mk_results = list(seasonalMannKendall(datetime, Value)),
    n_values = n(),
    mean = mean(Value, na.rm = TRUE),
    min = as.numeric(min(Value, na.rm = TRUE)),
    max = as.numeric(max(Value, na.rm = TRUE)),
    coefficient.of.variation = sd(Value, na.rm = TRUE) / mean(Value, na.rm = TRUE),
    .groups = 'drop'
  )
...
ggplot(sample_stats_df, aes(x = slope)) +
  ...
  labs(title = "Density Plot of Site Slopes (Log Scale)",
