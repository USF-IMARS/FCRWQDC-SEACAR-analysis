...
station_parameter_counts <- df %>%
  group_by(ProgramName, ProgramLocationID, ParameterName) %>%
...
station_depth_and_coords <- df %>%
  ...
  summarise(
    ...
    # calculate average coordinates for each station
...
dashboard_stations <- station_parameter_counts %>%
  ...
  left_join(station_depth_and_coords, by = c("ProgramName", "ProgramLocationID")) %>%
